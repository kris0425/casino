...
try{
  for(const sample of [{name:'mobile',viewport:{width:390,height:844}},{name:'desktop',viewport:{width:1280,height:900}}]){
    ...
    const cameraMoved=Math.abs(afterRight.cameraX-before.cameraX)>.05||Math.abs(afterRight.cameraZ-before.cameraZ)>.05;
    report.controls={before,afterUp,afterRight,screenUp:afterUp.y>before.y,screenRight:afterRight.x<afterUp.x,cameraMoved};
    ...
    if(report.mode!=='3D 模式'||!report.webgl||!report.overlayHidden||report.body>report.viewport||!report.controls.screenUp||!report.controls.screenRight||!report.controls.cameraMoved||errors.length)throw new Error(`${sample.name} visual check failed`);
    await page.close();
  }
