...
try{
  for(const sample of [{name:'mobile',viewport:{width:390,height:844}},{name:'desktop',viewport:{width:1280,height:900}}]){
    ...
    const report=await page.evaluate(()=>({art:document.querySelectorAll('.tile-art').length,sources:[...new Set([...document.querySelectorAll('.tile-art')].map(image=>image.getAttribute('src')))],mapBackground:/map-(spring|summer|autumn|winter)-(day|night)\.webp/.test(getComputedStyle(document.querySelector('.metro-viewport')).backgroundImage),mapTheme:document.querySelector('#metroViewport')?.dataset.theme||'',worldPhase:document.querySelector('#worldPhase')?.textContent||'',hall:Boolean(document.querySelector('.city-hall-art')?.complete),loading:document.querySelector('#loading')?.hidden,body:document.body.scrollWidth,viewport:document.documentElement.clientWidth}));
    await page.screenshot({path:resolve(outputRoot,`${sample.name}.png`),fullPage:true});
    console.log(`${sample.name}: ${JSON.stringify({...report,errors})}`);if(report.art<12||report.sources.length<10||!report.mapBackground||!report.mapTheme||!report.worldPhase||!report.hall||!report.loading||report.body>report.viewport||errors.length)throw new Error(`${sample.name} visual check failed`);await page.close();
  }
